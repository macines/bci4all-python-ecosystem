...
class CsvWriterWithTimestamp(IONode):
    ...
    Esta escolha foi adotada porque o formato transposto não é natural
    para escrita incremental linha a linha.
    """
    ...
    def __init__(self, file_name: str, sampling_rate: int, channel_count: int = 10):
        ...
        # sampling_rate -> usado para reconstruir o tempo relativo
        self.sampling_rate = int(sampling_rate)

        # channel_count -> número total de canais no sinal final
        self.channel_count = int(channel_count)

        # sample_idx -> contador global de amostras gravadas
        self.sample_idx = 0

        # start_dt -> instante absoluto de início da gravação
        self.start_dt = None
        ...
        # Buffers em memória:
        # time_values      -> vetor dos tempos relativos
        # timestamp_values -> vetor dos timestamps absolutos
    ...
    def _reset_buffers(self):
        ...
        É chamado quando chega o comando START_CSV.
        """
        self.sample_idx = 0
    ...
    def step(self, data):
        ...
        - normaliza o bloco;
        - reconstrói tempo relativo e timestamp absoluto;
        ...
        for row in arr:
            # Tempo relativo estimado pela posição da amostra no fluxo
            time_s = self.sample_idx / self.sampling_rate

            # Timestamp absoluto reconstruído a partir do instante de início
            ts = self.start_dt + timedelta(seconds=time_s)
            ts_str = ts.strftime("%Y-%m-%d %H:%M:%S.%f")

            self.time_values.append(f"{time_s:.3f}")
    ...
    def start_recording(self):
        ...
        Reinicializa os buffers e marca o writer como ativo.
        """
        self._reset_buffers()
